...
def mutualInformation(trainDataInput, prevAttributes, pathAttr):
...
def majority_vote(data):
    ...
    if len(unique_labels)!=1 :
        ...
        if counts[0] == counts[1]:
            ...
            majority_vote_label = unique_labels[0]
...
class Node:
    ...
    def growTree(self, data, maxDepth, pathAttr):
        ...
        if self.depth < maxDepth and len(self.prevAttributes) < colNames.size - 1 and self.nodeAttribute is not None:
            ...
            self.left.growTree(data[data[0:, self.nodeAttribute] == self.leftbranch], maxDepth, pathAttr)


           # self.prevAttributes = [self.prevAttributes[:self.depth - 1]]

            #right node
            self.right = Node(data[data[0:, self.nodeAttribute] == self.rightbranch], maxDepth, self.depth, self.prevAttributes)
            self.right.growTree(data[data[0:, self.nodeAttribute] == self.rightbranch], maxDepth, pathAttr)
...
def train(trainDataInput, maxDepth):
    ...
    tree = root.growTree(trainDataInput, maxDepth, None)
    return tree
...
# inputTrainingFileName = 'mushroom_train.tsv'
# inputTestFileName = 'mushroom_test.tsv'
# testOutputName = 'mushroom_3_test.LABELS'
# trainingOutputName = 'mushroom_3_train.LABELS'
# metricsFileName = 'mushroom_3_metrics.txt'
# max_depth = 6
...
#printing first line of pretty print
print(" ")
uniqueOutputLabels, countsInEachOutputLabel = np.unique(trainDataInput[:, -1], return_counts=True)
if uniqueOutputLabels.size == 1:
    missingvar = list(set(predictionLabels) - set(uniqueOutputLabels))
    uniqueOutputLabels = np.append(uniqueOutputLabels, missingvar, axis=0)
    countsInEachOutputLabel = np.append(countsInEachOutputLabel, 0)
print( '[', countsInEachOutputLabel[0],' ', uniqueOutputLabels[0], '/', countsInEachOutputLabel[1], ' ', uniqueOutputLabels[1], ']')
